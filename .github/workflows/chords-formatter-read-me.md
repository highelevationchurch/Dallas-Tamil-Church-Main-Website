...
- **Trigger:** Runs on every `push` to the `main` branch **only if** files within the `Songs/Chords/` directory have been changed. This includes **adding new `.txt` files** and **editing existing ones**.
- **Action:** Executes the `Songs/Chords/chords_formatter.js` script.

#### How `chords_formatter.js` Works

This Node.js script is responsible for the conversion. Its job is to:

1.  **Scan the `Songs/Chords/` directory** for `.txt` files.
2.  **Check for changes** by comparing the modification times of the `.txt` files and their corresponding `.html` files. It will only process a file if the `.html` version is missing or outdated.
3.  **Apply Formatting:**
...
4.  **Generate an HTML File:** It embeds the formatted content into a simple HTML structure and saves it with an `.html` extension.

#### The Formatting Process

Here’s a typical sequence:

1.  A user adds or edits a `.txt` chord chart in the `Songs/Chords/` directory.
...
5.  The script scans the directory, finds the new or updated `.txt` file, and generates a corresponding `.html` file with proper styling.
